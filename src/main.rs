...
use clearscreen;
use task::Task;
use todo::{add_new_task, display_todo_list, check_task, display_task_details, edit_task, delete_task};
use database::{write_todo_list, read_todo_list};

fn main() {
    // create list
    let mut todo_list: Vec<Task> = Vec::new();
    // read in todo_list from csv    
    todo_list = read_todo_list(true).expect("Could not read csv!");
    ...
    loop{
        ...
        println!("s - show unchecked tasks");
        println!("a - add a task");
        println!("d - show details of a task");
        println!("c - check or uncheck (toggle) task");
        println!("h - show checked tasks");
